...
const getSubscribtion = async (req, res) => {
    try {
        ...
        const subscribtion = await sequelize.query(
            `SELECT chanels.id, chanels.email, chanels.chanelName, chanels.description, chanels.cover, chanels.photo FROM "Subscribes" JOIN "Chanels" on subscribes.chanelId = chanels.id WHERE (subscriberId = ${id})`,
            {
